...
public class InCallScreen extends Activity
    ...
    protected void onStop() {
	    ...
	    if (mVoiceRecorder.getState() == 1 ){
		    mVoiceRecorder.stop();
		    stopRecordTimer();
		    //log("------------------[lkj] onStop: call mVoiceRecorder.stop();...");
	    }
        }
    }
	...
	private void startRecordTimer() {
        if (mCallCard != null) mCallCard.startRecordTimer(mCM.getActiveFgCall());
    }

    private void stopRecordTimer() {
        if (mCallCard != null) mCallCard.stopRecordTimer();
    }
	...
	if (mVoiceRecorder.getState() == 1 ){
		mVoiceRecorder.stop();
		stopRecordTimer();
		//log("------------------[lkj] onDisconnect: call mVoiceRecorder.stop();...");
	}
	...
	/*
     * onRecordClick is called only when there is a foreground call
     */
    private void onRecordClick() {
        int curRecordState = mVoiceRecorder.getState();
        log("onRecordClick: curRecordState = " + curRecordState);
        //PhoneUtils.setMute(newMuteState);
        if(curRecordState != 1)
        {
	...
	/*Get voice record state*/
	/* package */ int getVoiceRecordState()
	{
		return mVoiceRecorder.getState();
	}
	...
	if (mVoiceRecorder.getState() == 1 ){
		mVoiceRecorder.stop();
		stopRecordTimer();
		//log("------------------[lkj] internalHangup: call mVoiceRecorder.stop();...");
	}
