...
public class CallCard extends LinearLayout
        implements CallTime.OnTickListener, RecordTime.OnTickListener, CallerInfoAsyncQuery.OnQueryCompleteListener,
    ...
    private TextView mRecordElapsedTime;
    ...
    public CallCard(Context context, AttributeSet attrs) {
		...
		mRecordTime = new RecordTime(this);
	...
	public void onTickForRecordTimeElapsed(long timeElapsed) {
        // While a call is in progress, update the elapsed time shown
        // onscreen.
        updateRecordTimeWidget(timeElapsed);
    }
	...
	void startRecordTimer(Call call) {
        mRecordTime.setActiveCallMode(call);
        mRecordTime.reset();
        mRecordTime.periodicUpdateTimer();
        mRecordElapsedTime.setText("");
        mRecordElapsedTime.setVisibility(View.VISIBLE);
    }
    ...
    protected void onFinishInflate() {
	    ...
	    mRecordElapsedTime = (TextView) findViewById(R.id.recordTime);
    ...
    private void updateCallStateWidgets(Call call) {
        ...
        switch (state) {
            ...
            default:
		        ...
		        AnimationUtils.Fade.hide(mRecordElapsedTime, View.INVISIBLE);
	...
	private void updateRecordTimeWidget(long timeElapsed) {
        // if (DBG) log("updateElapsedTimeWidget: " + timeElapsed);
        if (timeElapsed == 0) {
            mRecordElapsedTime.setText("");
        } else {
            mRecordElapsedTime.setText(DateUtils.formatElapsedTime(timeElapsed));
        }
    }
