...
Value* MakeDirFn(const char* name, State* state, int argc, Expr* argv[]) {
		char* Path;
    if (argc != 1) {
        return ErrorAbort(state, "%s() expects 1 arg, got %d", name, argc);
    }
...
Value* SymlinkFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    int i;
    for (i = 0; i < argc-1; ++i) {
				if(!access(srcs[i],F_OK)){
					fprintf(stderr,"%s already exist continue\n",srcs[i]);
					continue;
				}
...
void RegisterInstallFunctions() {
    ...
    RegisterFunction("mkdir",MakeDirFn);
