...
static void select_input_device(struct sunxi_audio_device *adev)
{
	...
	ALOGV("select_input_device: in_device %d",adev->in_device);
	...
	ALOGV("select_input_device, devices: %x, mode: %x", adev->in_device, adev->mode);
...
static int start_output_stream(struct sunxi_stream_out *out)
{
	...
	ALOGD("start_output_stream: enter\n");
	...
	if (ret > 0)
	{
		...
		{
			ALOGD("unknown audio.routing : %s", prop_value);
		}
	}
	else
	{
		 ALOGD("get audio.routing failed");
	}

	//change the audio path for A31 C40 board, by zb,2013-12-20.
	if(device == AUDIO_DEVICE_OUT_SPEAKER)
		device = AUDIO_DEVICE_OUT_WIRED_HEADSET;

#if 0
	//ALOGD("force to:  AUDIO_DEVICE_OUT_SPEAKER");
	//device = AUDIO_DEVICE_OUT_SPEAKER;
	
	ALOGD("force to:  AUDIO_DEVICE_OUT_WIRED_HEADSET");
	device = AUDIO_DEVICE_OUT_WIRED_HEADSET;
#endif		

	ALOGD("####change out_device: %d\n", device);
	adev->out_device = device;

    adev->active_output = out;

    if (adev->mode != AUDIO_MODE_IN_CALL) {
		ALOGD("select_output_device ...\n");
...
static int start_input_stream(struct sunxi_stream_in *in)
{
	...
	ALOGD("start_input_stream: enter\n");
	...
	else
	{
		...
		ALOGD("out/in stream should be both 44.1K serial, force capture rate: %d", in_ajust_rate);
	}

	ALOGD("pcm_open_req: calling\n");
                                  ...
                                  struct audio_stream_in **stream_in)
{
    ...
    *stream_in = NULL;
	...
	ALOGD("adev_open_input_stream: exit\n");
                                   ...
                                   struct audio_stream_in *stream)
{
	...
	//ALOGV("adev_close_input_stream: enter\n");
