        	...
        	else
        	{
				if(pSelf->m_input_step == STEP_QEQUEST_INPUT_BUFFER)
				{
					...
					if(pSelf->m_useAndroidBuffer)
					{
						...
						unsigned char *img;
						res = GraphicBufferMapper::get().lock(pSelf->m_input_buffer_info.imgBuffer,
								GRALLOC_USAGE_SW_WRITE_OFTEN,
								rect, (void**)&img);
						if (res != OK) {
							ALOGE("%s: Unable to lock image buffer %p for access", __FUNCTION__, pSelf->m_input_buffer_info.imgBuffer);
							GraphicBufferMapper::get().unlock(pSelf->m_input_buffer_info.imgBuffer);
						}

						GraphicBufferMapper::get().get_phy_addess(pSelf->m_input_buffer_info.imgBuffer, &PhyAddr);
